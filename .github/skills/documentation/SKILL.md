...
# 27. 强调方式：不要用 Markdown 加粗

C++ 注释里禁止使用 Markdown 加粗语法：

```text
**文字**
```

原因（已实测，Doxygen 1.9.8 + clangd 18.1.3）：

| 查看方式 | `**词**` 的实际显示 |
| --- | --- |
| VS Code 编辑器 / GitHub 代码视图 | 原样显示星号（代码文件不渲染 Markdown） |
| clangd 悬浮提示 | clangd 会转义 `*`、`\b`、`<b>`、反引号，悬浮里永远没有加粗 |
| Doxygen HTML | Doxygen 只认 `\b 单个词` 或 `<b>文字</b>`，`**词**` 原样输出 |
...
* 需要强调的词或短语，用 「中文直角引号」 包住。
* 引号内不要再套 「」；必须嵌套时内层用 『』。
* 「」 紧贴前一个中文字符，前面不加空格（Markdown 加粗才需要那个空格）。
* 行内代码、标识符仍用反引号包裹，Doxygen 会渲染成代码字体。
* `docs/*.md` 等 Markdown 文档不受此限制，那里可以用 `**`。
