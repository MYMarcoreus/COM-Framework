...
CExampleTimerModule::CExampleTimerModule(std::int64_t intervalMs)
...
/// @brief 销毁定时器模块。
CExampleTimerModule::~CExampleTimerModule()
{
    Stop();
}
...
bool CExampleTimerModule::Initialize(const sc::CResolveContext& ctx)
{
...
bool CExampleTimerModule::Start()
{
    if (m_pTimer == nullptr)
    {
        return false;
...
/// @brief 停止定时器并释放接口引用。
void CExampleTimerModule::Shutdown()
{
    Stop();
    m_pTimer.Reset();
}
