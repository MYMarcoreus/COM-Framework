...
void CExampleAsyncModule::RunExample()
{
    ...
    if (nr.HasValue())
    {
        common::log::CLogger::Instance().Info("[AsyncExample] 链式=" + std::to_string(nr.Value()));
    }
    ...
    {
        common::log::CLogger::Instance().Info("[AsyncExample] fan-out 结果=" + std::to_string(v));
    });
    t.OnSuccess([](const int& v)
    {
        common::log::CLogger::Instance().Info("[AsyncExample] fan-out 第二个消费者=" + std::to_string(v));
    });
    t.Get();
    ...
    if (!nf.HasValue())
    {
        common::log::CLogger::Instance().Warn("[AsyncExample] 异常转无值: reason=" +
