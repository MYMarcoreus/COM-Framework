...
bool CExampleService::Initialize(const sc::CResolveContext& ctx)
{
    // ① 网络接口（发送响应）
...
void CExampleService::OnAccept(sc::ConnectionId id, const std::string& peer)
{
...
/// 并更新连接上下文与指标。
void CExampleService::OnData(sc::ConnectionId id, const char* data, size_t len)
{
    if (m_pRouter != nullptr)
    {
...
void CExampleService::OnClose(sc::ConnectionId id)
{
    if (m_pRouter != nullptr)
    {
...
/// 未配置异步执行器时退化为同步处理。
void CExampleService::HandlePing(sc::ConnectionId id)
{
...
void CExampleService::HandleEcho(sc::ConnectionId id, const char* payload, size_t payloadSize)
{
...
/// @brief 状态报告。
std::string CExampleService::GetStatus() const
{
    return "service";
}

/// @brief 记录日志。
void CExampleService::Log(const std::string& message)
{
