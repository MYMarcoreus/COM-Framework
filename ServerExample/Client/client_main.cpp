...
///
/// 连接 ServerExample 服务器，发送 PING 与 ECHO 请求并打印响应。
///
...
int main(int argc, char* argv[])
{
    ...
    // ② 发送 PING，期望 PONG
    std::string ping = serverexample::CExampleProtocol::BuildPing();
    DoRequest(fd, ping, "PING");

    // ③ 发送 ECHO，期望相同负载
    std::string echo = serverexample::CExampleProtocol::BuildPacket(serverexample::kCmdEcho, "Hello ServerCore");
...
void DoRequest(int fd, const std::string& request, const char* label)
{
    ...
    len = ntohl(len);
    if (len < 1 || len > serverexample::CExampleProtocol::kMaxPacketSize)
    {
