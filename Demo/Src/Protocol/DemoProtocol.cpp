...
///
/// @return 解析结果。
ParseResult DemoProtocol::ParsePacket(const std::string& data, size_t* consumed, Packet* packet)
{
...
///
/// @return 编码后的原始字节。
std::string DemoProtocol::BuildPacket(std::uint8_t command, const std::string& payload)
{
...
/// @brief 构建 PING 请求。
std::string DemoProtocol::BuildPing()
{
    return BuildPacket(kCmdPing, "ping");
}

/// @brief 构建 PONG 响应。
std::string DemoProtocol::BuildPong()
{
