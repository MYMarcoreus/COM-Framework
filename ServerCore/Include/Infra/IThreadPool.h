...
/// @brief 线程池接口（组件化适配 common::ThreadPool）。
///
...
/// @brief 线程池组件。
///
/// 内部持有 common::ThreadPool 实例。
class ThreadPoolComponent : public Component, public IThreadPool
{
public:
    explicit ThreadPoolComponent(size_t threadCount = 1);

    virtual ~ThreadPoolComponent();
