...
public:
    ...
    // 同一接口注册多个实例时返回第一个注册的模块。
    IModule* GetModuleByIid(const InterfaceId& iid) const;

    // 根据接口标识获取全部模块（支持同一接口的多实例注册）。
    std::vector<IModule*> GetModulesByIid(const InterfaceId& iid) const;

    // 指定接口标识的模块数量（多实例注册后 > 1）。
    size_t ModuleCountByIid(const InterfaceId& iid) const;

    // 按接口标识解析模块并返回指定接口类型的指针（借用指针）。
    // 内部完成 GetModuleByIid + QueryInterface + 类型转换，未找到返回 nullptr。
    // 用法：m_moduleManager.Resolve<sc::INetwork>(sc::IID_INetwork())。
    template <typename T>
    T* Resolve(const InterfaceId& iid) const
    {
        IModule* pModule = GetModuleByIid(iid);
        if (pModule == nullptr)
        {
            return nullptr;
        }
        return static_cast<T*>(pModule->QueryInterface(iid));
    }
...
private:
    ...
    // 依赖缺失 / 存在环时该模块退化为注册顺序位置，保证稳定。
    std::vector<size_t> ComputeStartOrder() const;
    ...
    // 接口标识 → 模块索引（multimap：同一接口支持多实例注册）。
    std::multimap<InterfaceId, size_t> m_mapIndexByIid;
