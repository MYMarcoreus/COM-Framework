...
public:
    ...
    // 初始化模块（创建资源、加载配置；依赖通过 ctx 注入）。
    virtual bool Initialize(const CResolveContext& ctx) = 0;
    ...
    // 本模块依赖的接口标识列表（供 CModuleManager 生命周期拓扑排序；依赖在前）。
    // 默认无依赖；CModule 派生类通过 AddDependency 声明。
    virtual const std::vector<InterfaceId>& GetDependencies() const
    {
        static const std::vector<InterfaceId> kEmpty;
        return kEmpty;
    }
};
