...
/// @brief 类型 → 接口标识 映射（主模板前向声明）。
///
/// 供 CResolveContext::Resolve<T>() 自动绑定，也供接口映射宏
/// SC_INTERFACE_ENTRY 自动推导接口标识（等同 rtcsdk 的 get_guid<T>()）。
/// 特化由各接口头的 SC_INTERFACE 宏自动生成，无需在此手动维护。
template <typename T>
