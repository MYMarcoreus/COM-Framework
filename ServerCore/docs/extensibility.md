...
3. 在业务服务 `Initialize` 中：
   - `router->SetExtractor(Protocol::MakeMessageExtractor())`
   - `router->RegisterHandler(kCmdXxx, handler)`
...
bool CMyApp::RegisterModules()
{
    ...
    // 业务模块
    if (!m_moduleManager.RegisterModule(sc::IID_INetworkHandler(), new CMyService()))
        return false;
    // ...
    return true;
}
