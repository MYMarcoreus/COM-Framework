...
/// @brief 内置事件类型常量。
///
/// 集中定义，避免事件类型作为魔法字符串散落在发布 / 订阅代码中。
namespace events {

// 网络模块生命周期事件（由 CTcpServerModule 发布）。
constexpr const char* const kNetworkStarted = "network.started";
constexpr const char* const kNetworkStopped = "network.stopped";
