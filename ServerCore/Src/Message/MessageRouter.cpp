...
/// @brief 创建消息路由器。
MessageRouter::MessageRouter() : nextId_(1)
{
}
...
void MessageRouter::SetExtractor(const MessageExtractor& extractor)
{
...
SubscriptionId MessageRouter::RegisterHandler(int type, const MessageHandler& handler)
{
    if (!handler)
    {
...
bool MessageRouter::UnregisterHandler(SubscriptionId id)
{
    std::lock_guard<std::mutex> lock(mutex_);
...
void MessageRouter::OnData(ConnectionId id, const char* data, size_t len)
{
...
/// @brief 连接关闭时清理缓冲。
void MessageRouter::OnClose(ConnectionId id)
{
    std::lock_guard<std::mutex> lock(mutex_);
    buffers_.erase(id);
}
...
void MessageRouter::Dispatch(ConnectionId id, int type,
...
/// @brief 接口查询实现。
bool MessageRouter::QueryInterfaceImpl(const InterfaceId& iid, void** ppv)
{
