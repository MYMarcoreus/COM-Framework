...
SubscriptionId EventDispatcher::Subscribe(const EventType& type, const EventHandler& handler)
{
    if (!handler)
    {
...
///
/// @return 实际接收事件的处理器数量。
size_t EventDispatcher::Publish(const EventType& type, const void* data, size_t size)
{
...
size_t EventDispatcher::SubscriberCount(const EventType& type) const
{
    std::lock_guard<std::mutex> lock(mutex_);
    std::map<EventType, std::vector<SubscriptionId> >::const_iterator it = byType_.find(type);
    if (it == byType_.end())
...
/// @brief 接口查询实现。
bool EventDispatcher::QueryInterfaceImpl(const InterfaceId& iid, void** ppv)
{
