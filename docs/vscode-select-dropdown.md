...
```json
{
    ...
    "tasks": [
        ...
        {
            "label": "build debug (Common + examples)",
            "type": "shell",
...
- `build debug (Common + examples)`：若被 `launch.json` 的 `preLaunchTask` 引用则**必须保留**，否则 F5 调试报错。
    ...
    "configurations": [
        {
            ...
            "cwd": "${workspaceFolder}",
            "preLaunchTask": "build debug (Common + examples)"   // 启动前自动构建
        }
    ]
}
```
