# 序列化（Common/Serialization）— 使用文档

> 实现细节见：[serialization-impl.md](serialization-impl.md)

Common 提供轻量二进制序列化基础设施（**小端字节序，带边界检查**），用于构建 / 解析二进制协议负载或持久化数据。

## 1. CBinaryWriter（内存写入缓冲）
...
## 2. CBinaryReader（顺序读取，带边界检查）

数据不足时返回 `false` 并置 `Failed()` 标记（**位置保持不动**），调用方可决定丢弃整条消息。
...
- 读者**借用**输入缓冲，读取期间缓冲必须保持存活；
- 空字符串 / 空字节串合法（长度为 0）。

## 4. 与协议的关系

序列化是字段级编码工具；完整报文（长度头 + 命令 + 负载）的切分由[消息流水线](../servercore/messaging-usage.md)的
提取器完成。两者组合：提取器切出负载 → 负载内用 `CBinaryReader` 解字段。
