...
| 业务错误（参数非法、查不到、权限不足…） | **返回值 / 异常 / 日志**（异步里是 `CPromiseResult` 携带的标准异常） |
...
| `Common/Async/PromiseCore.h`：`CPromise` 私有构造（句柄） | 共享核心非空、所指层状态非空（句柄恒指向一个层） |
| `Common/Async/PromiseLayer.h`：`MakeThenRunner` / `MakeResultRunner`；`Common/Async/PromiseCore.h`：`RunThenHandler` / `RunResultHandler` | 上下文 / 层状态非空 |
...
- **为什么不用「返回值 + 兜底」替代断言**：那是把编程错误伪装成可恢复错误；
  但如果调用方**确实**能处理（例如执行器已停 → 链以「执行器已停」收口），就必须走拒绝（异常），
  不能断言。
