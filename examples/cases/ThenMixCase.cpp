...
struct COrderContext {
  ...
  bool
      bFailReserve; ///< 演示开关：内层链「确认预占」强制拒绝（用来看拒绝如何透传）。
  std::atomic<int>
      nBillingDone; ///< 记账旁支是否完成（旁支晚于主链，用原子避免竞争）。
  std::string strTrace; ///< 执行轨迹。

  COrderContext()
      : nSku(0), nQty(0), nUnitPrice(0), nTotal(0), nStock(0),
        bStockEnough(false), bAudited(false), bCompensated(false),
        bFailReserve(false), nBillingDone(0) {}
};
...
///
/// @return bCond。
bool Expect(bool bCond, const std::string &strWhat,
...
///
/// @return 兑现。
no::CPromiseResult StepLoadOrder(no::CPromiseResult /*upResult*/,
                                 const std::shared_ptr<COrderContext> &spCtx) {
  std::this_thread::sleep_for(
      std::chrono::milliseconds(2)); // 模拟查商品库 / 缓存
...
/// @return 兑现。
no::CPromiseResult
StepApplyDiscount(no::CPromiseResult /*upResult*/,
                  const std::shared_ptr<COrderContext> &spCtx) {
...
///
/// @return 兑现。
no::CPromiseResult StepSaveOrder(no::CPromiseResult /*upResult*/,
                                 const std::shared_ptr<COrderContext> &spCtx) {
...
no::CPromiseResult StepCompensate(no::CPromiseResult upResult,
                                  const std::shared_ptr<COrderContext> &spCtx) {
  // catch 层：只在被拒绝时执行，upResult 必定是拒绝 —— 只有这里才需要看它。
  if (upResult.Code() ==
      kCodeOutOfStock) // 只有库存不足需要释放 / 回滚，参数非法无需补偿
  {
...
///
/// @return upResult。
no::CPromiseResult StepAudit(no::CPromiseResult upResult,
                             const std::shared_ptr<COrderContext> &spCtx) {
...
/// 层（库存模块）：建连。
static no::CPromiseResult
StepConnectStock(no::CPromiseResult /*upResult*/,
                 const std::shared_ptr<CStockContext> &spCtx) {
...
/// 层（库存模块）：读可售量。
static no::CPromiseResult
StepReadStock(no::CPromiseResult /*upResult*/,
              const std::shared_ptr<CStockContext> &spCtx) {
...
public:
  ...
  CStockPromise QueryStockAsync(int nSku) {
    ...
    spStock->nSku = nSku;
    return m_exec.NewPromise(spStock, &StepConnectStock, ASYNC_LOC)
        .Then(&StepReadStock, ASYNC_LOC);
  }
...
static no::CPromiseResult
StepWriteBilling(no::CPromiseResult /*upResult*/,
...
public:
  CBillingModule() : m_exec(1) { m_exec.Start(); }
  ...
  CBillingPromise WriteBillingAsync(int nOrderId, int nAmount) {
    std::shared_ptr<CBillingContext> spBill =
        std::make_shared<CBillingContext>();
...
COrderPromise
BridgeQueryStock(no::CAsyncExecutor &exec,
  ...
  COrderPromise::PromiseExecutor fnExecutor =
      [spStockModule, spCtx](const COrderPromise::ResolveFn &fnResolve,
                             const COrderPromise::RejectFn &fnReject) {
        // 发起跨模块调用：拿到的是「库存模块上下文的
        // promise」，执行器在模块内部。
        CStockPromise promiseStock =
            spStockModule->QueryStockAsync(spCtx->nSku);
        const std::shared_ptr<CStockContext> spStock =
            promiseStock.GetContext();
        promiseStock.OnSettled([spCtx, spStock, fnResolve,
                                fnReject](no::CPromiseResult result) {
          // 注意：本回调跑在**库存模块的线程**上 → 只做轻活（语义转换 +
          // 改上下文 + settle）。 写数据仍不会竞争：本回调 → fnResolve() →
          // 外层下一层，三步串行（依赖边给出 happens-before）。
          if (result.IsRejected()) {
            fnReject(
                result.Code()); // 库存模块拒绝 → 本流程拒绝（此处原样透传）。
            return;
          }
...
/// 层（内层链第 1 步）：预占库存。
static no::CPromiseResult
StepReserveStock(no::CPromiseResult /*upResult*/,
                 const std::shared_ptr<COrderContext> &spCtx) {
  spCtx->strTrace += "预占;";
  return no::CPromiseResult::Resolve();
}
...
/// 用来看内层拒绝如何沿外层链透传）。
static no::CPromiseResult
StepReserveConfirm(no::CPromiseResult /*upResult*/,
                   const std::shared_ptr<COrderContext> &spCtx) {
...
COrderPromise
BuildOrderFlow(no::CAsyncExecutor &exec,
               const std::shared_ptr<COrderContext> &spCtx,
               const std::shared_ptr<CStockModule> &spStockModule,
  ...
  COrderPromise::PromiseFactory fnQueryStock =
      [&exec, spStockModule](const std::shared_ptr<COrderContext> &spCtxSelf) {
  ...
  //    COrderContext）；
  //         跨上下文（如库存模块）需要先桥接（见 ③）。
  COrderPromise::PromiseFactory fnReserveInner =
      [&exec](const std::shared_ptr<COrderContext> &spCtxSelf) {
        return exec
  ...
  COrderPromise::ThenHandler fnBillingSideBranch =
      [spBillingModule](no::CPromiseResult /*upResult*/,
                        const std::shared_ptr<COrderContext> &spCtxSelf) {
        const int nOrderId = spCtxSelf->nSku * 1000 + spCtxSelf->nQty;
        CBillingPromise promiseBill =
...
static void WaitSideBranch(const std::shared_ptr<COrderContext> &spCtx,
                           int nTimeoutMs) {
  const int nStepMs = 2;
  for (int nWaited = 0;
       nWaited < nTimeoutMs &&
       spCtx->nBillingDone.load(std::memory_order_relaxed) == 0;
       nWaited += nStepMs) {
...
bool RunThenMixCase() {
  ...
  auto spStockModule =
      std::make_shared<CStockModule>(); // 库存模块（自持执行器）
  auto spBillingModule =
      std::make_shared<CBillingModule>(); // 记账模块（自持执行器）
  if (!exec.Start()) {
  ...
  const no::CPromiseResult resultOk =
      BuildOrderFlow(exec, spOk, spStockModule, spBillingModule).Await();
  WaitSideBranch(spOk, 1000); // ⑤ 是旁支：主链不等它，这里等一下再断言

  const bool bOkStock = (spOk->nStock == 5 && spOk->bStockEnough);
  const bool bOkTrace =
      (spOk->strTrace == "读订单;校验通过;查库存(5);满减;预占;确认预占;"
                         "记账已发起(不等);落库;审计;");
  bOk = Expect(resultOk.IsFulfilled(), "正常下单：最终兑现",
               "码=" + std::to_string(resultOk.Code())) &&
        bOk;
  bOk = Expect(spOk->nTotal == 3375, "正常下单：合计=1250×3×0.9=3375",
               "实际=" + std::to_string(spOk->nTotal)) &&
        bOk;
  bOk = Expect(bOkStock, "正常下单：库存 5 件足够",
               "库存=" + std::to_string(spOk->nStock)) &&
        bOk;
  bOk = Expect(spOk->nBillingDone.load() == 1, "正常下单：旁支记账已完成",
               "nBillingDone!=1") &&
        bOk;
  bOk = Expect(spOk->bAudited, "正常下单：finally 审计已执行",
               "bAudited=false") &&
        bOk;
  bOk = Expect(bOkTrace, "正常下单：执行顺序符合预期",
               "轨迹=" + spOk->strTrace) &&
        bOk;
  ...
  const no::CPromiseResult resultOut =
      BuildOrderFlow(exec, spOut, spStockModule, spBillingModule).Await();

  const bool bOkOutCode =
      (resultOut.IsRejected() &&
       resultOut.Code() == static_cast<int>(kCodeOutOfStock));
  const bool bOkOutTrace =
      (spOut->strTrace == "读订单;校验通过;查库存(5);库存不足;补偿;审计;");
  bOk = Expect(bOkOutCode, "库存不足：拒绝码为 kCodeOutOfStock",
               "码=" + std::to_string(resultOut.Code())) &&
        bOk;
  bOk = Expect(spOut->bCompensated, "库存不足：catch 补偿已执行",
               "bCompensated=false") &&
        bOk;
  bOk = Expect(spOut->nTotal == 0, "库存不足：④ 算折扣未执行",
               "nTotal=" + std::to_string(spOut->nTotal)) &&
        bOk;
  bOk = Expect(bOkOutTrace, "库存不足：执行顺序符合预期",
               "轨迹=" + spOut->strTrace) &&
        bOk;
  ...
  const no::CPromiseResult resultBad =
      BuildOrderFlow(exec, spBad, spStockModule, spBillingModule).Await();

  const bool bOkBadCode = (resultBad.IsRejected() &&
                           resultBad.Code() == static_cast<int>(kCodeBadOrder));
  const bool bOkBadTrace =
      (spBad->strTrace == "读订单;校验失败(单笔最多 10 件);审计;");
  bOk = Expect(bOkBadCode, "参数非法：拒绝码为 kCodeBadOrder",
               "码=" + std::to_string(resultBad.Code())) &&
        bOk;
  bOk = Expect(bOkBadTrace, "参数非法：失败即停（未查库存、未记账）",
               "轨迹=" + spBad->strTrace) &&
        bOk;
  bOk = Expect(spBad->nBillingDone.load() == 0, "参数非法：旁支未被发起",
               "nBillingDone!=0") &&
        bOk;
  ...
  const no::CPromiseResult resultInner =
      BuildOrderFlow(exec, spInner, spStockModule, spBillingModule).Await();

  const bool bOkInnerCode =
      (resultInner.IsRejected() &&
       resultInner.Code() == static_cast<int>(kCodeReserveFailed));
  const bool bOkInnerTrace =
      (spInner->strTrace == "读订单;校验通过;查库存(5);无折扣;预占;审计;");
  bOk = Expect(bOkInnerCode, "内层链拒绝：拒绝码透传到外层",
               "码=" + std::to_string(resultInner.Code())) &&
        bOk;
  bOk =
      Expect(bOkInnerTrace, "内层链拒绝：外层后续 then 不执行、finally 仍执行",
             "轨迹=" + spInner->strTrace) &&
      bOk;
  bOk = Expect(spInner->nBillingDone.load() == 0, "内层链拒绝：后续旁支未发起",
               "nBillingDone!=0") &&
        bOk;
