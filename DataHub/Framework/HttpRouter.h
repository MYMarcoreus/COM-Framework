...
// 路由处理函数：接收请求任务与完整请求路径（已去 query），返回是否已写响应。
typedef std::function<bool(WFHttpTask*, const std::string&)> HttpHandler;
...
struct HttpRoute
{
    ...
    HttpHandler handler;  // 处理器（可为 lambda / std::bind / 静态函数）
};
...
class CHttpRouter
{
   public:
    ...
    bool Dispatch(WFHttpTask* pServerTask, const std::string& strMethod, const std::string& strPath);
