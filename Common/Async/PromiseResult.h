...
//
// **判「谁产生的」只看具体码**：框架码就是 `kRejected` / `kStopped` / `kException` 三个，
// 别拿「码 >= kBusinessBase」这类区间判断当分类依据 —— 框架不校验码值，那只是业务侧的编号习惯。
...
/// @brief 业务错误码的**建议**起点：100。
///
/// 框架**从不读它、也不校验任何码值** —— 它只做一件事：给业务一个不会撞上框架码的编号起点
/// （真正被占用的是 1 / 2 / 3：`Reject(码)` 会给它们附上框架固定文案；0 可以当业务码用）。
/// 所以它**不是分类依据**：要区分「框架失败 / 业务失败」请直接判具体码
/// （框架码 = `码 >= kRejected && 码 <= kException`）。
