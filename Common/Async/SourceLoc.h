...
// __PRETTY_FUNCTION__ / __FILE__ / __LINE__ 存进那一层的记录（`CLayerInfo::loc`），
// 于是 trace 的 `DescribeLayer` / 测试断言能直接告诉你「这一层是哪个函数、哪个文件、
// 第几行注册的」—— 同一处代码往往注册多个 lambda，注册点行号是定位「哪一层」最直接的手段。
//
...
///
/// 仅在调试构建下由层记录（`CLayerInfo::loc`）保存，供 trace / 测试断言定位注册点；
/// 发布构建下不存（零开销）。
struct CSourceLoc
{
    ...
    CSourceLoc(const char* pszFunction, const char* pszFile, int nLine) : szFunction(pszFunction), szFile(pszFile), nLine(nLine)
    {}
};
