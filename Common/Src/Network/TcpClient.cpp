                        ...
                        const CloseCallback& closeCb)
{
    ...
    running_.store(true);
    thread_ = std::thread(&TcpClient::ThreadMain, this);
    return true;
}
...
/// @brief 处理连接完成。
void TcpClient::HandleConnect(const asio::error_code& ec, const asio::ip::tcp::endpoint& endpoint)
{
    if (ec)
    {
        if (connectCb_)
...
/// @brief 处理读完成。
void TcpClient::HandleRead(const asio::error_code& ec, size_t bytes)
{
    if (ec)
    {
...
/// @brief 追加待发送数据并启动写。
void TcpClient::AppendWrite(const std::string& data)
{
    if (!socket_.is_open())
    {
        return;
...
/// @brief 处理写完成。
void TcpClient::HandleWrite(const asio::error_code& ec, size_t bytes)
{
    if (ec)
    {
        if (ec != asio::error::operation_aborted)
...
/// @brief 在 io 线程内关闭连接。
void TcpClient::CloseOnIoThread()
{
    if (socket_.is_open())
    {
