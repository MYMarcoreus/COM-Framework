...
void CLogger::SetMaxFileSize(std::uint64_t nMaxBytes)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    m_nMaxFileBytes = nMaxBytes;
}
...
/// @brief 按大小滚动日志文件。
///
/// 当前文件超过上限时：旧备份依次后移（.N-1 → .N），当前文件改名为 .1，
/// 然后重新打开新文件继续写入。持有 m_mutex 时调用。
void CLogger::RotateIfNeeded()
{
    if (!m_file.is_open() || m_strFilePath.empty())
    {
        return;
    }
    if (FileSize(m_strFilePath) < m_nMaxFileBytes)
    {
        return;
    }
    m_file.close();
    // 滚动备份：kMaxBackupCount-1 → kMaxBackupCount，...，1 → 2
    ...
    std::remove(strBackup.c_str());
    std::rename(m_strFilePath.c_str(), strBackup.c_str());
    // 重新打开新文件
    m_file.open(m_strFilePath.c_str(), std::ios::out | std::ios::app);
    if (!m_file.is_open())
    {
        m_bFileEnabled = false;
    }
...
/// @brief 返回文件字节数；失败返回 0。
std::uint64_t CLogger::FileSize(const std::string& strPath)
{
    struct stat st;
